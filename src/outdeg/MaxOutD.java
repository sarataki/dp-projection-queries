...
public class MaxOutD {
	//Compute maxOutdegree of nodes after projection. GS=D
		/*Projection while bounding max outDegree of nodes- EdgeOrdering: OrderLexicographical
		OutedgePrivacy+ TO (BoundOutdegree)
	 Compute maxOutdegree of nodes. */
	public static void main(String[] args) {
		System.out.println("MaxOutDegree");
		Graph myG = new Graph("tweets.rdf");
		System.out.println("UNBOUNDED EDGES");
		Graph myOtherG = new Graph();
		myOtherG.setNodes(myG.getNodes());
		ArrayList<Edge> sortedEdges = new ArrayList<Edge>(myG.getEdges());
		
		myOtherG.setEdges(sortedEdges);

		System.out.println("SORTED UNBOUNDED EDGES Accroding to the lexicographical order");
		
		BoundedOutDegree mBG = new BoundedOutDegree(myG, new OrderLexicographical(), Integer.parseInt(args[0]));
		...
		try {
			String queryString = "Select (Max(?outdegree) as ?maxoutdegree) WHERE{ Select ?outdegree (count(*) as ?count) WHERE{ SELECT ?s  (count(*) as ?outdegree) WHERE{ ?s ?p ?o}Group By ?s ORDER BY DESC(?outdegree)} Group By ?outdegree ORDER BY (?outdegree)}";
			...
			for (; results.hasNext();) {
				...
				System.out.println(x3 + "   " + finalAns[1] + "  " + finalAns[2] + "  " + finalAns[3] + "  "
						+ finalAns[4] + "  " + finalAns[5] + "  " + finalAns[6]);*/
