...
public class AlphQL {
	//Max tweet Outdegree
  //QLOutedge Privacy+TQL Bound QLoutdegree of nodes- QL={tweeted, hasName, RDF.type}-OrderAlph

	public static void main(String[] args) {
		System.out.println("LexQL");
		Graph myG = new Graph("tweets.rdf");
		System.out.println("UNBOUNDED EDGES");

		// according to the Alphab
		Set<String> labels=new HashSet<String>();
		labels.add("http://rdfanon.org/types#tweeted");
		labels.add("http://rdfanon.org/types#name");
		BoundedQlOutDegree mBG = new BoundedQlOutDegree(myG, new OrderAlphabeticalOrderS(), Integer.parseInt(args[0]),labels);
		...
		try {
			String queryString = " Select (Max(?outdegree) as ?maxoutdegree) WHERE{ Select ?outdegree (count(*) as ?count) WHERE{ SELECT ?s  (count(*) as ?outdegree) WHERE{ ?s <http://rdfanon.org/types#tweeted> ?o}Group By ?s ORDER BY DESC(?outdegree)} Group By ?outdegree ORDER BY (?outdegree)}";
